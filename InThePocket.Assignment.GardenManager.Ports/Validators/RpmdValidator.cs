...
public class RpmdValidator : AbstractValidator<RealtimePlantMetricDataDto>
{
    public RpmdValidator()
    {
        ...
        RuleFor(x => x.LastIrrigationEndTime).LessThan(DateTime.Now).WithMessage("LastIrrigationEndTime must be in the past.");
        RuleFor(x => x.LastIrrigationEndTime)
            .GreaterThan(x => x.LastIrrigationStartTime)
            .WithMessage("LastIrrigationEndTime must be after LastIrrigationStartTime.");
    }
